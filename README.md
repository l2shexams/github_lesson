...
> все файлы в репозитории возвращаются в состояние, в котором они находились на момент указанного коммита. Если перед переходом в репозиторий были внесены изменения, которые были добавлены в репозиторий, но не попали в коммит — они будут перенесены «поверх» состояния после перехода.
> 
...
В верней части окна показываются файлы, которые были изменены или созданы и отслеживаются git. 'Unversioned 
...
Рядом с нашим коммитом мы можем увидеть две бирочки: желтую и зеленую. Зеленая — веткa main, желтая — HEAD.
...
Теперь загрузим изменения из нашего локального репозитория в сетевой (удаленный) репозиторий. Для этого используется команда Git → Push (Ctrl + Shift + k). В появившимся окне мы увидим, какие коммиты отправляются из локального в удаленный.

![](https://yastatic.net/s3/lyceum/content/presentation/photo_2021-11-12%2013.24.03.jpeg)

При выполнении этих действий будет запрошен ваши логин GitHub и токен авторизации. Токен можно получить в Developer 
...
По логам видно, что у нас есть три ветки: main, newBranch и demobranch. demobranch — текущая активная ветка, то, что мы находимся в ней, подтверждается указателем HEAD. Также есть ветка newBranch, у которой с demobranch общий источник кода — последний на текущий момент коммит ветки main.
...
Если мы зайдем на GitHub, мы увидим, что изменения которые мы делали, там не отразились. Это нормально, ведь все изменения мы делали локально. Давайте запушим нашу ветку printOne:
...
Cделаем коммит. После этого переключимся на ветку main, убедимся, что файл вернулся к изначальной версии, и создадим еще одну ветку uppercase. Стоя на ней, изменим файл следующим образом:
...
Cнова зафиксируем версию. Вернемся на ветку main и вольем в нее изменения из ветки lowercase. Слияние должно пройти успешно, а на ветке main появятся маленькие буквы. После этого выполним слияние с веткой uppercase. Теперь возник конфликт.
